/* 此文件来源于 npm rc-select 中的 Select.js, 并作一定修改 */
/* eslint-disable */
...
} from 'rc-select/lib/util';
import SelectTrigger from 'rc-select/lib/SelectTrigger';
import FilterMixin from 'rc-select/lib/FilterMixin';
...
const Select = React.createClass({
    propTypes: {
        ...
        // 自定义新属性
        autoFocus: PropTypes.bool, // 是否自动聚焦输入框 (仅在showSearch下生效)
        alwaysShowPopup: PropTypes.bool, // 强制总是显示下拉弹出框
        alwaysFireSearch: PropTypes.bool, // 只要搜索框文字发生变动, 就触发 onSearch (比如多选且可搜索情况下, 选择/取消选择时输入文本会置空, 但是默认不触发 onSearch)
        onOptionHighlight: PropTypes.func // 当移动上下键导致高亮选项发生变更时触发, 参数为 (highlightOption)
    },
    ...
    onInputKeyDown(event) {
        ...
        if (state.open) {
            ...
            if (menu && menu.onKeyDown(event)) {
                ...
                event.stopPropagation();
                this.setState({}, () => {
                    const {activeKey} = menu.state,
                        activeItem = _.find(menu.getFlatInstanceArray(), item => {
                            return item.props.eventKey === activeKey;
                        });
                    activeItem && this.props.onOptionHighlight(activeItem);
                });
            }
        }
    },
    ...
    setInputValue(inputValue, fireSearch = true) {
        ...
        if (fireSearch || this.props.alwaysFireSearch) {
    ...
    onTopControlNodeClick() {
        if (this.props.alwaysShowPopup) {
    ...
    renderTopControlNode() {
        ...
        return (<div className={className} onClick={this.onTopControlNodeClick}>{this.getPlaceholderElement()}{innerNode}</div>);
    },
    ...
    render() {
        ...
        return (
            <SelectTrigger
                ...
                multiple={multiple}
                disabled={disabled}
                visible={open || (state._isMounted && props.alwaysShowPopup)}
